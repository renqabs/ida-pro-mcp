...
class Server:
    ...
    def __init__(self):
        ...
        mcp.settings.host = Server.HOST
        mcp.settings.port = Server.PORT
    ...
    def _run_server(self):
        try:
            ...
            print(f"[MCP SSE]  Server started at http://{Server.HOST}:{Server.PORT}/sse")
            mcp.run('sse')
        except OSError as e:
            if e.errno == 98 or e.errno == 10048:  # Port already in use (Linux/Windows)
                print("[MCP SSE]  Error: Port 13337 is already in use")
            else:
